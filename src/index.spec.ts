    ...
    describe(`(${JSON.stringify(source)})`, () => {
        ...
        if (expectedVertexCount === 1) {
            ...
            it("should have all of the vertices from the arcs, and only those vertices", () => {
                const expectedVertices = new Set()
                for (const arc of result.graph[1]) {
                    expectedVertices.add(arc[0]).add(arc[1])
                }
                expectSetsEqual(result.graph[0], expectedVertices)
            })
        }
...
describe("The Newick string parser", () => {
    describe("when given an example from the specification", () => {
        testNewick({
            ...
            expectedWritten:
                "((((cat:47.14069,monkey:100.8593):20.59201,weasel:18.87953):2.0946,(sea_lion:11.997,seal:12.003):7.52973):3.87382,(bear:6.80041,raccoon:19.19959):0.846,dog:25.46154);",
            source: "((raccoon:19.19959,bear:6.80041):0.84600,((sea_lion:11.99700, seal:12.00300):7.52973,((monkey:100.85930,cat:47.14069):20.59201, weasel:18.87953):2.09460):3.87382,dog:25.46154);",
        })
        testNewick({
            expectedVertexCount: 12,
            expectedRootWeight: 0.1,
            expectedWritten:
                "(((((Chimp:0.19268,Human:0.11927):0.08386,Gorilla:0.17147):0.06124,Orang:0.33636):0.15057,Gibbon:0.36079):0.54939,Bovine:0.69395,Mouse:1.2146);",
            source: "(Bovine:0.69395,(Gibbon:0.36079,(Orang:0.33636,(Gorilla:0.17147,(Chimp:0.19268, Human:0.11927):0.08386):0.06124):0.15057):0.54939,Mouse:1.21460):0.10;",
        })
        testNewick({
            expectedVertexCount: 12,
            expectedWritten:
                "(((((H._sapiens:0.11927,P._paniscus:0.19268):0.08386,G._Gorilla:0.17147):0.06124,Pongo:0.33636):0.15057,Hylobates:0.36079):0.54939,Bovine:0.69395,Rodent:1.2146);",
            source: "(Bovine:0.69395,(Hylobates:0.36079,(Pongo:0.33636,(G._Gorilla:0.17147, (P._paniscus:0.19268,H._sapiens:0.11927):0.08386):0.06124):0.15057):0.54939, Rodent:1.21460);",
        })
        testNewick({
            expectedRootLabel: "A",
    ...
    describe("when given the example from the `README`", () => {
        testNewick({
            ...
            source: "(Pongo:15.76,(Gorilla:9.06,(Pan:6.65,Homo:6.65):2.41)Homininae:6.70)Hominidae:4.43;",
        })
    })
