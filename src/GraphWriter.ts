...
export default class GraphWriter {
    constructor(private readonly graph: Graph) {
        ...
        for (const arc of graph[1]) {
            rootCandidates.delete(arc[1])
            ;(this.outgoing.get(arc[0]) as Set<[Vertex, number]>).add(arc.slice(1, 3) as [Vertex, number])
        }
    ...
    private getSort(vertex: Vertex): string {
        ...
        const children = Array.from(this.outgoing.get(vertex) as Set<[Vertex, number]>)
